...
class Absolute():  
    def __init__(self, initial_law = np.ones(7)/7, data_file = "Data/history_processed.xlsx", scenarios_path = "Data/scenarios.xlsx", 
        ...
        year_cols = [c for c in rates.columns if str(c).isdigit()]
        rates = rates[year_cols]
        ...
        for scenar in mus.index:
            ...
            while last_year < 2050:  
                max_year = f"{last_year+1}"
                sce[max_year] = mus.loc[scenar, max_year] + sce[f"{last_year}"]
                sce = sce.copy()
                last_year += 1
...
#%%
    ...
    def get_density(self, theta, dt, scenario = 0, t = 0):
        ...
        if t+self.start_year > self.ref_year:
            a_t = np.sum(self.mus.loc[:, '2024':str(t+self.start_year + 1)], axis = 1)
            a_t = a_t @ self.probas
        
            shift = shift + a_t
            
        #cov = (a_t + theta[3+self.n:3+2*self.n]) * np.eye(self.n)
        cov = (theta[3+self.n:3+2*self.n]) * np.eye(self.n)
        
        #cov = a_t + theta[0] * (self.last["Scope12"].values**2 / np.sum(self.last["Scope12"].values**2)) * np.eye(self.n)
        
        # Np array 
        mean = self.scenario_emissions[scenario, :, t] + shift

        return(multivariate_normal.pdf(dt, mean = mean, cov = cov))
