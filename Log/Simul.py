...
central_var = Config.VAR_BIS
beta = Config.BETA
nus = Config.NUS_BIS
sigmas = Config.SIGMAS_BIS

nus_df = Config.NUS_NEW
sigmas_df = Config.SIGMAS_NEW
kappa = Config.KAPPA
...
def simul_order(central_var, beta, kappa, nus_df, sigmas_df, scenar_index = None, scenar_name = None, mus = final_df):
    n = len(sigmas)
    ...
    # apply a_t directly
    a_t = np.prod(1+mu_old/100)
    shift = nus_df["Spreads"].values * a_t
    
    mean = (mu_old * np.abs(emissions) / 100) + emissions
    
    matrix = a_t**2 * (1+ kappa) * (sigmas_df["Variances"].values * np.eye(n))
    ...
    for col in mus.columns[mus.columns > 2024].values:
        ...
        a_t = np.prod(1+mus.loc[locmu, 2024:col+1]/100)
        ...
        # matrix = (central_var + beta * (dt - mu_old)**2) * np.ones((n,n))
...
def compare_simul_mus(dis, scenar_index, scenar_name = None, mus = final_df):
    ...
    for t in years:
        ...
        mus_slice = mus_selected.loc[2022:t] / 100
...
#sim_dis, sim_mus = simulate_N_dis_and_mus(N,central_var,beta,nus_df,scenar_index,scenar_name=None, mus=final_df)
#plot_mus_and_simul(sim_mus = sim_mus, mus = final_df, scenar_name = scenar_name, alpha=0.95)

for scenar_used in all_scenar:
    
     dis = simul_order(central_var, beta, kappa, nus_df, sigmas_df, scenar_name = scenar_used)
