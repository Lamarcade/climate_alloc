# -*- coding: utf-8 -*-
"""
Created on Mon Dec 15 15:06:59 2025
...
def simul_order(central_std, beta, nus_df, sigmas_df, scenar_index = None, scenar_name = None, mus = final_df):
    n = len(sigmas)
    ...
    dis = pd.DataFrame({2024: di}, index = Config.HISTO_ORDER)
...
def compare_simul_mus(dis, scenar_index, scenar_name = None, mus = final_df):
    ...
    compar = pd.DataFrame(index=sectors, columns=years)
...
#sim_dis, sim_mus = simulate_N_dis_and_mus(N,central_std,beta,nus_df, sigmas_df, scenar_index,scenar_name=None, mus=final_df)
